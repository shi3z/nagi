...
### テキスト入力
`Text`ボタンで入力フィールドを開き、日本語などのCJK文字を入力できます。

> **iOSユーザーへの注意**: ATOKなどのサードパーティIMEは、xterm.jsのcompositionイベントの制限により正しく動作しません。日本語入力には`Text`ボタンをご利用ください。iOS標準キーボードは問題なく動作します。
